...
public class PrivacyPolicyTests
{
    ...
    public async Task AcceptedPolicy_SetsFlagAndMovesToPin()
    {
        ...
        var result = await ctx.Service.AcceptPrivacyPolicyAsync(
            new AcceptPrivacyPolicyRequest(registration.Id, true));
        ...
        Assert.True(dbRegistration!.IsPrivacyAccepted);
    }

    [Fact]
    public async Task RejectedPolicy_ReturnsPrivacyNotAcceptedAndDoesNotChangeState()
    {
        using var ctx = new RegistrationTestContext();
        var registration = await ctx.AddRegistrationAsync(
            status: RegistrationStatuses.PendingPrivacyPolicy,
        ...
        var result = await ctx.Service.AcceptPrivacyPolicyAsync(
            new AcceptPrivacyPolicyRequest(registration.Id, false));

        Assert.False(result.IsSuccess);
        Assert.Equal("PRIVACY_NOT_ACCEPTED", result.Code);

        var dbRegistration = await ctx.GetRegistrationAsync(registration.Id);
        Assert.False(dbRegistration!.IsPrivacyAccepted);
        Assert.Equal(RegistrationStatuses.PendingPrivacyPolicy, dbRegistration.Status);
    }
    ...
    public async Task UnknownRegistration_ReturnsNotFound()
    {
        ...
        var result = await ctx.Service.AcceptPrivacyPolicyAsync(
            new AcceptPrivacyPolicyRequest(Guid.NewGuid(), true));
    ...
    [Fact]
    public async Task AcceptedPolicy_DoesNotChangeVerificationFlags()
    {
        using var ctx = new RegistrationTestContext();
        var registration = await ctx.AddRegistrationAsync(
            status: RegistrationStatuses.PendingPrivacyPolicy,
